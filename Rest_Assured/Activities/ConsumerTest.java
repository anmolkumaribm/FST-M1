...
public class ConsumerTest {
	...
	public V4Pact createPostFragment(PactDslWithProvider builder)
	{
		...
		return builder.given("POST Request")
				...
				.willRespondWith().status(201).body(reqResBody)
				.toPact(V4Pact.class);
	}
	...
	public V4Pact createGetAllFragment(PactDslWithProvider builder) {
		...
		return builder.given("GET ALL Request")
				...
				.path("/api/users")
				.headers(headers)
				.willRespondWith().status(200).body(arrResBody)
				.toPact(V4Pact.class);
	}
	...
	public void postRequestTest(MockServer mockServer) {
		...
		given().baseUri(mockServer.getUrl() + "/api/users").headers(headers).body(reqBody).log().all().when().post()
	...
	public void getRequestTest(MockServer mockServer) {
		// Send request, get response, assert response
		given().baseUri(mockServer.getUrl() + "/api/users").headers(headers).log().all().when().get("/1").then()
	...
	public void deleteRequestTest(MockServer mockServer) {
		// Send request, get response, assert response
		given().baseUri(mockServer.getUrl() + "/api/users").headers(headers).log().all().when().delete("/1").then()
	...
	public void getAllRequestTest(MockServer mockServer) {
		// Send request, get response, assert response
		given().baseUri(mockServer.getUrl() + "/api/users").headers(headers).log().all().when().get().then()
				.statusCode(200).log().all();
	}
}
